...
public class HackedTwitterStreamImpl extends TwitterBaseImpl implements TwitterStream {
    private static final long serialVersionUID = 5621090317737561048L;
    private final HttpClient http;
    private static final Logger logger = Logger.getLogger(HackedTwitterStreamImpl.class);
    ...
    /*package*/
    HackedTwitterStreamImpl(String theConsumerKey, Configuration conf, Authorization auth) {
        super(conf, auth);
        http = HttpClientFactory.getInstance(new twitter4j.StreamingReadTimeoutConfiguration(conf));
    ...
    private StatusStream getCountStream(String relativeUrl, int count) throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new HackedStatusStreamImpl(this.consumerKey, getDispatcher(), http.post(conf.getStreamBaseURL() + relativeUrl
    ...
    StatusStream getRetweetStream() throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new HackedStatusStreamImpl(this.consumerKey, getDispatcher(), http.post(conf.getStreamBaseURL() + "statuses/retweet.json"
    ...
    StatusStream getSampleStream() throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new HackedStatusStreamImpl(this.consumerKey, getDispatcher(), http.get(conf.getStreamBaseURL() + "statuses/sample.json?"
            ...
            synchronized (this) {
                if (null == this.dispatcher) {
                    ...
                    if (dispatcher instanceof DispatcherImpl) {
                        dispatcher = new HackedDispatcherImpl(conf);
                    }
                }
            }
        }
    ...
    private transient volatile Dispatcher dispatcher;
    ...
    StatusStream getFilterStream(FilterQuery query) throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new HackedStatusStreamImpl(this.consumerKey, getDispatcher(), http.post(conf.getStreamBaseURL()
    ...
    private static volatile int numberOfHandlers = 0;
    ...
    abstract class TwitterStreamConsumer extends Thread {
        ...
        public void run() {
            ...
            while (!closed) {
                try {
                    if (!closed && null == stream) {
                        ...
                        logger.info("Establishing connection [" + HackedTwitterStreamImpl.this.consumerKey + "].");
                        setStatus("[Establishing connection]");
                        stream = (StatusStreamBase) getStream();
                        connected = true;
                        logger.info("Connection established [" + HackedTwitterStreamImpl.this.consumerKey + "].");
                        ...
                        timeToSleep = NO_WAIT;
                        logger.info("Receiving status stream [" + HackedTwitterStreamImpl.this.consumerKey + "].");
            ...
            logger.info("Shutting down stream thread [" + HackedTwitterStreamImpl.this.consumerKey + "].");
        }

        public synchronized void close() {
            setStatus("[Disposing thread]");
            logger.info("Closing stream [" + HackedTwitterStreamImpl.this.consumerKey + "].");
            closed = true;
            if (stream != null) {
                try {
                    stream.close();
    ...
    public boolean equals(Object o) {
        ...
        HackedTwitterStreamImpl that = (HackedTwitterStreamImpl) o;
